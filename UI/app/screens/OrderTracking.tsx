...
export function OrderTracking() {
  ...
  const { cart = [], total = 0, orderId = 'DSU2847' } = location.state || {};
        ...
        <div className="max-w-md mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/60 text-xs">ORDER ID: #{orderId}</p>
            </div>
          </div>
        ...
        <div className="px-4 mb-4">
          <div className="bg-white rounded-2xl p-4" style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
            ...
            {cart.map((cartItem: any) => (
              <div key={cartItem.item.id} className="flex items-center gap-3 mb-3 last:mb-0">
                <img
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="text-sm text-gray-900 truncate" style={{ fontWeight: 600 }}>{cartItem.item.name}</h4>
                  <p className="text-xs text-gray-500">{cartItem.item.isVeg ? 'Regular' : 'Extra Spice'} · {cartItem.quantity} unit</p>
                </div>
                <span className="text-sm text-gray-900" style={{ fontWeight: 700 }}>₹{cartItem.item.price * cartItem.quantity}</span>
              </div>
            ))}
            <div className="mt-3 pt-3 flex items-center justify-between" style={{ borderTop: '1.5px dashed #E5E5E5' }}>
              ...
              <span className="text-gray-900" style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 700, fontSize: '18px' }}>₹{total}</span>
            </div>
          </div>
        </div>
